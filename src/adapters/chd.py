...
class ChdAdapter(Adapter):
    ...
    def _raise_for_login_error(html: str) -> None:
        ...
        # **只匹配页面可见文本，不匹配原始 HTML。**
        #
        # 登录页模板中始终包含 placeholder="请输入验证码" 与
        # <img alt="验证码错误">。若对原始 HTML 做子串匹配，「请输入验证码」
        # 在**未登录的空白登录页**上即可命中 4 次，导致任何一次页面重新渲染
        # 都被判定为需要验证码，其中也包括密码错误后重新渲染的页面。
        # 改用 get_text() 之后命中数为 0，因为属性值不属于文本内容。
        text = BeautifulSoup(html, "lxml").get_text(" ", strip=True)

        # 判定在这里作出，因此提示也要在这里给全：后两种的恢复方式与
        # 「密码错误」相反，**更换密码没有作用**，需要人工在网页端处理。
        for pattern, msg, needs_human in [
            # 认证服务器返回的原文是「您提供的用户名或者密码有误」。
            # 原先匹配的「用户名或密码错误」并不会出现在页面中，
            # 因此密码错误这一类此前从未被正确识别。
            (r"用户名或者?密码(有误|错误|不正确)", "用户名或密码错误", False),
            (r"账号已?被锁定|账号被停用",
             "账号已被锁定。**更换密码没有作用**——请前往 "
             "https://ids.chd.edu.cn 等待锁定解除并确认可以正常登录，"
             "再执行 --unlock-login 恢复。", True),
            # 不再匹配「请输入验证码」：那是输入框的占位提示，不是错误信息。
            # 验证码确实出错时，页面会给出可见的提示文字。
            (r"验证码(不正确|错误|有误)",
