...
#
# 顺带一个好处：拿全部学期意味着挂在旧学期下的补考、重修成绩也能抓到。
_GRADE_PATHS = [
...
# 这些词是别的列名的子串，只允许完全相等匹配，不许拿去做子串匹配。
...
class ChdAdapter(Adapter):
    ...
    def _resume_via_sso(self, login_url: str, params: dict) -> bool:
        ...
        系统，附上新 ticket——全程不碰密码。

        这条路径是必需的，不是优化：EAMS 的 JSESSIONID 大约一个多小时就失效
        一次，而以前每次失效都要完整重登。2026-08-16 账号被统一身份认证判为
        「频繁登录」冻结 10 分钟，根子就在这儿。

        另外，没有这条路径的话，7 天免登录一旦真的生效反而会把程序搞坏：
        ...
        if r.status_code not in _REDIRECT_CODES:
            return False          # 给的是登录表单，说明 TGT 没了，得走完整流程
    ...
    def _locate(headers: list[str]) -> dict[str, int]:
        ...
        2. 子串兜底，但跳过 _AMBIGUOUS 里那些是别的列名子串的词。
        """
    ...
    def _parse(cls, html: str) -> list[Grade]:
        ...
        #
        # 抛出去之后本轮整个失败：不推送、不更新快照、下轮重试，连续 5 轮推告警。
        # 代价是教务处改版时监控会停摆——但停摆会喊人，推错数据不会。
        #
        # 注意「最终」列回退**没有**一起升级：那个只影响分数取哪一列，有 WARNING
        # 看得见，而且改版时它是唯一还能让程序继续跑的退路。
        problems = []
        if incomplete:
            problems.append(
