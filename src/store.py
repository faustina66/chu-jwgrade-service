...
# 被旧代码按错误含义读取。错误解析比拒绝读取更危险。
SNAPSHOT_SCHEMA_VERSION = 1

# 一门课连着这么多轮没抓到，才判定为撤回。默认 1 = 发现就报。
#
# 长安大学的成绩以整行形式出现和消失：出分时课程行与分数同时出现，
# 撤回时整行不见、后面的行自动补位。所以"行还在但分数栏空了"这种情况根本
# 不存在，唯一的撤回信号就是行消失。
#
# 一开始默认是 2（等一轮再确认），用于防止页面内容不完整导致误报。
# 长期运行记录未发现单门课程随机缺失，而多等一轮会延迟撤回通知。
#
# 因此默认改为 1，发现后立即通知。如果实际运行中出现误报，可将
# safety.withdraw_confirm_rounds 调成 2，无需修改代码。
#
# 页面残缺由另外三项检查处理，与这个数无关：取到 0 条直接报错、
# 整个学期消失要多等一轮、一轮消失达到 max_withdrawals 判定页面异常。
ABSENT_ROUNDS_TO_CONFIRM = 1

# 整个学期都没出现时，改用这个更保守的确认轮数。
#
# 原来的规则是「整学期消失一律不算撤回」，用于防止页面残缺导致误报，
# 但存在一个盲区：
...
class GradeStore:
    ...
    def _foreign_reason(self, data: dict) -> str | None:
        ...
        - 换了学号。如果配置改成另一个账号，旧快照仍属于原账号，不能直接
          用来比较新账号的成绩。
...
def _vanished(old: dict[str, Grade], new: list[Grade]):
    ...
    前置检查：已经确认撤回的课程不再重复计数，否则每轮都会重新推送。
...
def diff(old: dict[str, Grade], new: list[Grade],
    ...
    1. 按行的存在与否。长安大学的成绩整行出现、整行消失，所以这是撤回的
       唯一信号。见 _vanished() 的前置检查和 ABSENT_ROUNDS_TO_CONFIRM。
