...
class PushPlus(Notifier):
    ...
    def _check(self, r: requests.Response) -> str | None:
        ...
        if code in self.FATAL:
            ...
            # 旧说明与实际含义相反。根据官方返回码表修正后，同时补充 900 的处理。
...
_CSS = (
    ...
    # .l、.e、.n、.c、.s 等短类名可能与宿主页面冲突。2026-08-17 的公众号和
    # 小程序测试均正常，因此暂时保留；如出现样式串扰，可改为 .jwl、.jwe 等
    # 带前缀的类名，但每门课程会增加十余字节。
    #
    ...
    # 未设置 class 的 <b> 标签。
    "<style>"
    # word-break 规则添加于 2026-08-19，用于解决手机上「最终100」被拆成
    ...
    # 默认样式同样可以正确显示满宽自动列。
    #
    # 2026-08-19 依次测试了带引号类名、内联 width 和 `.jw td` 后代选择器。
    # 前两种不能解决问题，后代选择器会被小程序识别并使第一列超出屏幕。
...
def _headline(c: Change, brief: bool = False) -> str:
    ...
    和绩点，以减少发送给第三方推送服务的内容。2026-08-20 之前此处未检查
    level，导致这些字段仍会发送，与配置说明不一致。
    """
